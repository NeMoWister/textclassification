...
def plot_phik(data, figsize=(10, 8)):
...
def plot_hist_numeric(data, feature, figsize=(8, 4), x_min=None, x_max=None, y_max=None): 
...
def plot_classification_results(metrics, model_name="Model"):
    ...
    if 'ROC Curve' in metrics:
        ...
        pr_data = metrics['Precision-Recall Curve']
        plt.subplot(1, 2, 2)
        plt.plot(
            pr_data['recall'], pr_data['precision'],
            color='blue', lw=2,
            label=f'PR curve (AP = {metrics["Average Precision"]:.2f})'
        )
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
...
def print_classification_report(metrics, model_name="Model"):
    ...
    metrics_df = pd.DataFrame({
        'Metric': ['ROC AUC', 'Average Precision', 'F1 Score', 'Precision', 'Recall', 'Accuracy'],
        'Value': [
            f'{metrics["ROC AUC"]:.4f}' if metrics["ROC AUC"] is not None else 'N/A',
            f'{metrics["Average Precision"]:.4f}' if metrics["Average Precision"] is not None else 'N/A', 
